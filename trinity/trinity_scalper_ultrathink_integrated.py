...
class TrinityUltraThinkScalper:
    ...
    async def execute_ultrathink_signal(self, signal: Dict):
        ...
        # Try stocks first if market open
        if market_hours['stocks'] and self.alpaca.connected:
            symbol = self.stocks[self.total_trades % len(self.stocks)]
            result = await self.alpaca.execute_trade(
                symbol, 
                1,  # 1 share to avoid PDT restrictions
                signal['signal']
            )
            if result['success']:
                trade_executed = True
        ...
        if not trade_executed and self.alpaca.connected:
